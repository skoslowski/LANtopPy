...
from gcalimport.event_parser import Action, extract_actions, \
...
class EventParserTest(unittest.TestCase):
    def test_parse_event(self):
        ...
        exp = [Action(events[0]["start"], {0: u"on"}, u"x ch0 adsf"),
               Action(events[0]["end"], {0: u"auto"}, u"x ch0 adsf")]
    ...
    def test_parse_desc(self):
        ...
        actions = list(extract_actions(events, CHANNELS))
        exp = [Action(events[0]["start"], {0: u"on"}, u"summary"),
               Action(events[0]["end"], {0: u"auto"}, u"summary"),
               Action(events[0]["start"], {2: u"on"}, u"summary"),
               Action(events[0]["end"], {2: u"auto"}, u"summary")]
        self.assertEqual(exp, actions)
    ...
    def test_parse_desc_offset(self):
        ...
        exp = [Action(events[0]["start"] + timedelta(minutes=60),
                                {1: u"on"}, u"summary"),
               Action(events[0]["end"], {1: u"auto"}, u"summary")]
        self.assertEqual(exp, actions)
